...
type
  TBusquedaEntradas = class(TForm)
    ...
    cAlbaranBus: TEdit;
    ClientDataSet1empresa_ce: TStringField;
    ClientDataSet1albaran_ce: TIntegerField;
    ClientDataSet1cliente_ce: TStringField;
    ClientDataSet1importe_entrada_ce: TFloatField;
    ClientDataSet1horas_entrada_ce: TFloatField;
    ClientDataSet1numero_factura_ce: TIntegerField;
    ClientDataSet1temperatura_ce: TSmallintField;
    ClientDataSet1concepto_ce: TStringField;
    ClientDataSet1tipo_entrada_ce: TStringField;
    ClientDataSet1transportista_ce: TStringField;
    ClientDataSet1matricula_ce: TStringField;
    ClientDataSet1procedencia_ce: TStringField;
    ClientDataSet1observaciones_ce: TStringField;
    ClientDataSet1concepto2_ce: TStringField;
    ClientDataSet1hora_ce: TStringField;
    ClientDataSet1fecha_albaran_ce: TStringField;
    ClientDataSet1FechaAlbaran: TStringField;
    ClientDataSet1FechaFactura: TStringField;
    ClientDataSet1fecha_factura_ce: TStringField;
...
var
   BusquedaEntradas: TBusquedaEntradas;
...
procedure TBusquedaEntradas.NuevaBusqueda(Sender: TObject);
begin
     Self.Height:=296;
     Self.width:=463;
...
procedure TBusquedaEntradas.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//--------------------- CONTROL DE PULSACIONES DE TECLADO ----------------------
procedure TBusquedaEntradas.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
//----------------------- BOTON ACEPTAR ----------------------------------------
procedure TBusquedaEntradas.Aceptar(Sender: TObject);
Var
   HayWhere: Boolean;
begin
     NRegistros:=0;
     ...
     begin
          ...
          begin
               ...
               begin
                    WhereBus:=' WHERE ( EMPRESA_CE=' + QuotedStr(RellenarCeros(cEmpresaBus.Text, cEmpresaBus.MaxLength, 'I'));
                    HayWhere:=True;
               end;
               if cClienteBus.Text<>'' then
               begin
                    if HayWhere then
                       WhereBus:=WhereBus + ' AND CLIENTE_CE=' + QuotedStr(RellenarCeros(cClienteBus.Text, cClienteBus.MaxLength, 'I'))
                    else
                    begin
                         WhereBus:=' WHERE ( CLIENTE_CE=' + QuotedStr(RellenarCeros(cClienteBus.Text, cClienteBus.MaxLength, 'I'));
                         HayWhere:=True;
                    end;
               end;
               ...
               begin
                    if HayWhere then
                       WhereBus:=WhereBus + ' AND ALBARAN_CE=' + QuotedStr(cAlbaranBus.Text)
                    else
                    begin
                         WhereBus:=' WHERE (ALBARAN_CE=' + QuotedStr(cAlbaranBus.Text);
                         HayWhere:=True;
                    end;
               end;
               ...
               begin
                    if HayWhere then
                       WhereBus:=WhereBus + ' AND FECHA_ALBARAN_CE BETWEEN ' + QuotedStr(AnyoMesDia(cFechaDesdeBus.Text)) + ' AND ' + QuotedStr(AnyoMesDia(cFechaHastaBus.Text))
                    else
                        WhereBus:=' WHERE ( FECHA_ALBARAN_CE BETWEEN ' + QuotedStr(AnyoMesDia(cFechaDesdeBus.Text)) + ' AND ' + QuotedStr(AnyoMesDia(cFechaHastaBus.Text));
               end;
               if TipoLote<>'' then
               begin
                    if HayWhere then
                       WhereBus:=WhereBus + ' AND TIPO_ENTRADA_CE=' + QuotedStr(TipoLote)
                    else
                        WhereBus:=' WHERE ( TIPO_ENTRADA_CE=' + QuotedStr(TipoLote);
               end;
               if WhereBus<>'' then
                  WhereBus:=WhereBus + ')';
               Query1.SQL.Add(WhereBus);
               TextoSql:=Query1.SQL.Text;
               Query1.SQL.Add(' ORDER BY EMPRESA_CE, CLIENTE_CE, ALBARAN_CE, FECHA_ALBARAN_CE');
               Query1.Open;
     ...
     begin
          EmpresaOK:=ClientDataSet1.FieldByName('EMPRESA_CE').AsString;
          AlbaranOK:=ClientDataSet1.FieldByName('ALBARAN_CE').AsInteger;
          ModalResult:=mrOk;
     end;
end;

//----------------------- BOTON CANCELAR ---------------------------------------
procedure TBusquedaEntradas.Cancelar(Sender: TObject);
begin
     ModalResult:=mrCancel;
end;

//----------------------------- DOBLE CLICK EN EL GRID -------------------------
procedure TBusquedaEntradas.GridLineasDblClick(Sender: TObject);
begin
     EmpresaBus:=ClientDataSet1.FieldByName('EMPRESA_CE').AsString;
     AlbaranBus:=ClientDataSet1.FieldByName('ALBARAN_CE').AsInteger;

     ModalResult:=mrOk;
end;

//--------------------- MENSAJES DE LA BARRA DE ESTADO -------------------------
procedure TBusquedaEntradas.Mensaje(Texto: String );
begin
...
procedure TBusquedaEntradas.Timer1Timer(Sender: TObject);
begin
     if BarraEstado.SimplePanel then
...
procedure TBusquedaEntradas.FormCreate(Sender: TObject);
begin
     Self.Height:=296;
     Self.width:=463;
     PosRegistro:=0;
...
procedure TBusquedaEntradas.GridLineasTitleClick(Column: TColumn);
begin
     ClientDataSet1.Close;
     Query1.Close;
     Query1.SQL.Clear;
     ...
     if Column.FieldName='FechaFactura' then
        Query1.SQL.Add(TextoSql +
                       ' ORDER BY FECHA_FACTURA_CE')
     else if Column.FieldName='FechaAlbaran' then
             Query1.SQL.Add(TextoSql +
                            ' ORDER BY FECHA_ALBARAN_CE')
...
procedure TBusquedaEntradas.ColorEnter(Sender: TObject);
begin
...
procedure TBusquedaEntradas.ColorExit(Sender: TObject);
begin
     if Sender is TEdit then           //Si el componente es un TDBEdit
     begin
...
procedure TBusquedaEntradas.cEmpresaBusChange(Sender: TObject);
Var
   Empresa: String;
begin
...
//-------------------------- PRIMER REGISTRO -----------------------------------
procedure TBusquedaEntradas.Primero(Sender: TObject);
begin
     ClientDataSet1.First;
end;

//-------------------------- REGISTRO ANTERIOR ---------------------------------
procedure TBusquedaEntradas.Anterior(Sender: TObject);
begin
     ClientDataSet1.Prior;
end;

//-------------------------- REGISTRO SIGUIENTE --------------------------------
procedure TBusquedaEntradas.Siguiente(Sender: TObject);
begin
     ClientDataSet1.Next;
end;

//-------------------------- ULTIMO REGISTRO -----------------------------------
procedure TBusquedaEntradas.Ultimo(Sender: TObject);
begin
     ClientDataSet1.Last;
end;

//-------------- HABILITAR BOTONES DE POSICION DE REGISTRO ---------------------
procedure TBusquedaEntradas.BotonesPosicion;
begin
...
procedure TBusquedaEntradas.ClientDataSet1AfterScroll(DataSet: TDataSet);
begin
...
procedure TBusquedaEntradas.cClienteBusChange(Sender: TObject);
begin
...
procedure TBusquedaEntradas.ClientDataSet1CalcFields(DataSet: TDataSet);
begin
     //Cambia el formato de la fecha de entrada al visualizar (YYYY/MM/DD --> DD/MM/YYYY)
     ClientDataSet1.FieldByName('FechaAlbaran').AsString:=DiaMesAnyo(ClientDataSet1.FieldByName('Fecha_Albaran_ce').AsString);
     ClientDataSet1.FieldByName('FechaFactura').AsString:=DiaMesAnyo(ClientDataSet1.FieldByName('Fecha_Factura_ce').AsString);
end;

//-------------------------- LUPA (F2) DE EMPRESAS -----------------------------
procedure TBusquedaEntradas.cEmpresaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_F2 then
...
//-------------------------- LUPA DE EMPRESAS ----------------------------------
procedure TBusquedaEntradas.cLupaEmpresasClick(Sender: TObject);
Var
   CodEmpresa: String;
begin
...
//-------------------------- LUPA DE EMPRESAS ----------------------------------
procedure TBusquedaEntradas.cLupaClientesClick(Sender: TObject);
Var
   CodCliente: String;
begin
     if cEmpresaBus.Text<>'' then
...
//-------------------------- LUPA (F2) DE CLIENTES -----------------------------
procedure TBusquedaEntradas.cClienteKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_F2 then
        cLupaClientesClick(Self);
end;

procedure TBusquedaEntradas.FormActivate(Sender: TObject);
begin
     cClienteBus.SetFocus;
end;

//-------------------------- VALIDACION DE LA SELECCION ------------------------
function TBusquedaEntradas.Validar():Boolean;
begin
     Result:=True;
