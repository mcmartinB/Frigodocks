...
uses
  Windows, SysUtils, quickrpt, Menus, FPrincipal, Printers, QRExport,
...
uses dlgConfigPrinter, QRPDFFilt;{, CVariables, DConfigMail, UDFactura, UDMConfig,
...
begin
  ...
  begin
    if ( Copy( LowerCase(TQuickRep(QRPrinter.master).Name), 1, 9)  = 'qralbaran' ) or
       ( LowerCase(TQuickRep(QRPrinter.master).Name) = 'qrlalboutput' ) then
    begin
