...
uses
  ...
  cxContainer, cxEdit, dxSkinsCore, dxSkinBlue, dxSkinsdxBarPainter, Menus,
  cxButtons, SimpleSearch, dxBar, cxClasses, cxTextEdit, cxDBEdit;

type
  TFamilias = class(TForm)
    ...
    QMaestroempresa_f: TStringField;
    QMaestrocodigo_f: TStringField;
    QMaestrodescripcion_f: TStringField;
...
uses UGLobal, MBaseDatos, DM, BFamilias;
...
procedure TFamilias.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     Action:=caFree;
end;

//----------------------- CIERRRE DEL QUERY DEL FORMULARIO ---------------------
procedure TFamilias.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
     CanClose:=True;
...
procedure TFamilias.Mensaje(Texto: String );
begin
...
procedure TFamilias.Timer1Timer(Sender: TObject);
begin
     if BarraEstado.SimplePanel then
...
procedure TFamilias.BotonesPosicion;
begin
...
//--------------------- BOTON ALTA ---------------------------------------------
procedure TFamilias.Alta(Sender: TObject);
begin
     swAlta:=True;
     PonerColorAlta();
...
//--------------------- BOTON BORRAR -------------------------------------------
procedure TFamilias.Borrar(Sender: TObject);
begin
...
//-------------------------- PRIMER REGISTRO -----------------------------------
procedure TFamilias.Primero(Sender: TObject);
begin
...
//-------------------------- REGISTRO ANTERIOR ---------------------------------
procedure TFamilias.Anterior(Sender: TObject);
begin
...
//-------------------------- REGISTRO SIGUIENTE --------------------------------
procedure TFamilias.Siguiente(Sender: TObject);
begin
...
//-------------------------- ULTIMO REGISTRO -----------------------------------
procedure TFamilias.Ultimo(Sender: TObject);
begin
...
//----------------------------- BUSQUEDA ---------------------------------------
procedure TFamilias.Busqueda(Sender: TObject);
Var
   Aux: TBusquedaFamilias;
begin
     try
        Aux:=TBusquedaFamilias.Create(Self);
        with Aux do
        begin
             AceptarTodos:=True;
...
//------------------------ BOTON ACEPTAR ----------------------------------------
procedure TFamilias.Aceptar(Sender: TObject);
Var
   Posicion: TBookmark;
begin
...
//--------------------- BOTON CANCELAR -----------------------------------------
procedure TFamilias.Cancelar(Sender: TObject);
begin
     if QMaestro.State in dsEditModes then
     begin
...
//----------------------------- SALIR ------------------------------------------
procedure TFamilias.Salir(Sender: TObject);
begin
     Close;
end;

//-------------------------- VALIDACION DE DATOS CABECERA ----------------------
function TFamilias.ValidarCabecera():Boolean;
begin
     Result:=True;
     ...
     else if (cCodigo.Text='') or (EsNumerico(cCodigo.Text) and  (StrToInt(cCodigo.Text)=0)) then
     begin
          Result:=False;
...
function TFamilias.ValidarDetalles():Boolean;
begin
     Result:=True;
end;

//------------------- FOCO EN EL PANEL DE DATOS --------------------------------
procedure TFamilias.PanelDatosEnter(Sender: TObject);
begin
...
function TFamilias.EjecutaQuery(Empresa, Codigo: String):Boolean;
begin
     Result:=False;
     NRegistros:=0;
     PosRegistro:=0;
     try
        QMaestro.Close;
        QMaestro.SQL.Clear;
        QMaestro.SQL.Add(' SELECT * FROM FAMILIAS ' +
                         ' WHERE EMPRESA_F=' + Quotedstr(Empresa) +
                         ' AND CODIGO_F=' + QuotedStr(Codigo) +
                         ' ORDER BY EMPRESA_F,CODIGO_F');
        QMaestro.Open;
        if QMaestro.IsEmpty then
           Result:=False
        else
        begin
             try    //Contamos los Registros
                ...
                begin
                     Close;
                     SQL.Clear;
                     SQL.Add('SELECT COUNT(*) AS NREGISTROS FROM FAMILIAS ' +
                             ' WHERE EMPRESA_F=' + Quotedstr(Empresa) +
                             ' AND CODIGO_F=' + QuotedStr(Codigo));
                     Open;
...
function TFamilias.EjecutaQueryBus(WhereBus: String):Boolean;
begin
     Result:=False;
     NRegistros:=0;
     PosRegistro:=0;
        ...
        begin
             try    //Contamos los Registros
                ...
                begin
                     Close;
                     SQL.Clear;
                     SQL.Add('SELECT COUNT(*) AS NREGISTROS FROM FAMILIAS ' + WhereBus);
                     Open;
                     NRegistros:=FieldByName('NRegistros').AsInteger;  //Contamos los registros
                     PosRegistro:=1;
                end;
...
//-------------------- EVENTO SCROLL -------------------------------------------
procedure TFamilias.AlMoverRegistro(DataSet: TDataSet);
begin
     if SwAlta=False then  //El estado es de visualizacion
     begin
          cEmpresa.Text:=QMaestro.FieldByName('EMPRESA_F').AsString;   //No estan enganchados a BD y debemos asignarlos a mano
          cCodigo.Text:=QMaestro.FieldByName('CODIGO_F').AsString;
     end;
end;

//--------------------- CAMBIO DE ESTADO DEL DATA SOURCE -----------------------
procedure TFamilias.DSMaestroStateChange(Sender: TObject);
begin
...
procedure TFamilias.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//--------------------- CONTROL DE PULSACIONES DE TECLADO ----------------------
procedure TFamilias.FormKeyPress(Sender: TObject; var Key: Char);
begin
     if Key=#13 then
     begin
          Key:=#0;
...
//---------------------- DESPUES DE INSERTAR -----------------------------------
procedure TFamilias.QMaestroAfterInsert(DataSet: TDataSet);
begin
     QMaestro.FieldByName('EMPRESA_F').AsString:=cEmpresa.Text;
     QMaestro.FieldByName('CODIGO_F').AsString:=cCodigo.Text;
end;

//---------------------- PONE EL COLOR DE FOCO DE LOS CAMPOS CLAVES ------------
procedure TFamilias.ColorEnter(Sender: TObject);
begin
...
procedure TFamilias.ColorExit(Sender: TObject);
begin
     if Sender is TEdit then           //Si el componente es un TDBEdit
     begin
          if not SwAlta then
...
procedure TFamilias.QuitarColorAlta();
begin
...
procedure TFamilias.PonerColorAlta();
begin
...
procedure TFamilias.cEmpresaChange(Sender: TObject);
begin
     tNomEmpresa.Caption:=NomEmpresa(RellenarCeros(cEmpresa.Text,cEmpresa.MaxLength,'I'));
end;

//-------------------------- LUPA (F2) DE EMPRESAS -----------------------------
procedure TFamilias.cEmpresaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_F2 then
...
//-------------------------- LUPA DE EMPRESAS ----------------------------------
procedure TFamilias.cLupaEmpresasClick(Sender: TObject);
Var
   CodEmpresa: String;
begin
...
procedure TFamilias.FormActivate(Sender: TObject);
begin
