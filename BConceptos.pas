...
type
  TBusquedaConceptos = class(TForm)
    Query1: TQuery;
    DS: TDataSource;
    Timer1: TTimer;
    PanelDatos: TPanel;
...
var
   BusquedaConceptos: TBusquedaConceptos;
...
procedure TBusquedaConceptos.NuevaBusqueda(Sender: TObject);
begin
     Self.Height:=187;
...
procedure TBusquedaConceptos.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//--------------------- CONTROL DE PULSACIONES DE TECLADO ----------------------
procedure TBusquedaConceptos.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
//----------------------- BOTON ACEPTAR ----------------------------------------
procedure TBusquedaConceptos.Aceptar(Sender: TObject);
begin
...
function TBusquedaConceptos.EjecutarQuery():Boolean;
Var
   HayWhere: Boolean;
begin
     Result:=False;
     ...
     begin
          WhereBus:=' WHERE EMPRESA_C=' + QuotedStr(cEmpresaBus.Text);
          HayWhere:=True;
     end;
     if cCodigoBus.Text<>'' then
     begin
          if HayWhere then
             WhereBus:=WhereBus + ' AND CODIGO_C=' + QuotedStr(cCodigoBus.Text)
          else
              WhereBus:=' WHERE CODIGO_C=' + QuotedStr(cCodigoBus.Text);
     end;
...
//----------------------- BOTON CANCELAR ---------------------------------------
procedure TBusquedaConceptos.Cancelar(Sender: TObject);
begin
     ModalResult:=mrCancel;
end;

//----------------------------- DOBLE CLICK EN EL GRID -------------------------
procedure TBusquedaConceptos.GridLineasDblClick(Sender: TObject);
begin
...
procedure TBusquedaConceptos.Mensaje(Texto: String );
begin
...
procedure TBusquedaConceptos.Timer1Timer(Sender: TObject);
begin
     if BarraEstado.SimplePanel then
...
procedure TBusquedaConceptos.FormCreate(Sender: TObject);
begin
     Self.Height:=187;
     Self.width:=470;
end;
...
procedure TBusquedaConceptos.FormActivate(Sender: TObject);
begin
     cEmpresaBus.Text:=EmpresaParam;
...
procedure TBusquedaConceptos.GridLineasTitleClick(Column: TColumn);
begin
     ClientDataSet1.Close;
     Query1.Close;
     Query1.SQL.Clear;
...
procedure TBusquedaConceptos.cEmpresaBusChange(Sender: TObject);
begin
     tNomEmpresa.Caption:=NomEmpresa(RellenarCeros(cEmpresaBus.Text,cEmpresaBus.MaxLength,'I'));
end;

//---------------------- PONE EL COLOR DEL FOCO --------------------------------
procedure TBusquedaConceptos.ColorEnter(Sender: TObject);
begin
...
procedure TBusquedaConceptos.ColorExit(Sender: TObject);
begin
     if Sender is TEdit then           //Si el componente es un TDBEdit
     begin
...
//-------------------------- PRIMER REGISTRO -----------------------------------
procedure TBusquedaConceptos.Primero(Sender: TObject);
begin
     ClientDataSet1.First;
end;

//-------------------------- REGISTRO ANTERIOR ---------------------------------
procedure TBusquedaConceptos.Anterior(Sender: TObject);
begin
     ClientDataSet1.Prior;
end;

//-------------------------- REGISTRO SIGUIENTE --------------------------------
procedure TBusquedaConceptos.Siguiente(Sender: TObject);
begin
     ClientDataSet1.Next;
end;

//-------------------------- ULTIMO REGISTRO -----------------------------------
procedure TBusquedaConceptos.Ultimo(Sender: TObject);
begin
     ClientDataSet1.Last;
end;

//-------------- HABILITAR BOTONES DE POSICION DE REGISTRO ---------------------
procedure TBusquedaConceptos.BotonesPosicion;
begin
...
procedure TBusquedaConceptos.ClientDataSet1AfterScroll(DataSet: TDataSet);
begin
...
//-------------------------- LUPA (F2) DE EMPRESAS -----------------------------
procedure TBusquedaConceptos.cEmpresaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_F2 then
...
//-------------------------- LUPA DE EMPRESAS ----------------------------------
procedure TBusquedaConceptos.cLupaEmpresasClick(Sender: TObject);
Var
   CodEmpresa: String;
begin
