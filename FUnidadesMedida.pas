...
uses
  ...
  cxContainer, cxEdit, dxSkinsCore, dxSkinBlue, Menus, dxSkinsdxBarPainter,
  dxBar, cxClasses, cxButtons, SimpleSearch, cxTextEdit, cxDBEdit;

type
  TUnidadesMedida = class(TForm)
    ...
    cDenAbreviada: TcxDBTextEdit;
    StaticText12: TStaticText;
    cEmpresa: TEdit;
    tNomEmpresa: TPanel;
    QMaestroempresa_u: TStringField;
    QMaestrocodigo_u: TStringField;
    QMaestrodescripcion_u: TStringField;
    QMaestrodes_abreviada_u: TStringField;
...
uses UGLobal, MBaseDatos, DM, BUnidadesMedida;

{$R *.dfm}

//------------------------ CREACION DEL FORMULARIO -----------------------------
procedure TUnidadesMedida.FormCreate(Sender: TObject);
begin
     Top:=1;
     Left:=0;
...
procedure TUnidadesMedida.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     Action:=caFree;
end;

//----------------------- CIERRRE DEL QUERY DEL FORMULARIO ---------------------
procedure TUnidadesMedida.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
     CanClose:=True;
...
procedure TUnidadesMedida.Mensaje(Texto: String );
begin
...
procedure TUnidadesMedida.Timer1Timer(Sender: TObject);
begin
     if BarraEstado.SimplePanel then
...
procedure TUnidadesMedida.BotonesPosicion;
begin
...
//--------------------- BOTON ALTA ---------------------------------------------
procedure TUnidadesMedida.Alta(Sender: TObject);
begin
     swAlta:=True;
     PonerColorAlta();
...
//--------------------- BOTON BORRAR -------------------------------------------
procedure TUnidadesMedida.Borrar(Sender: TObject);
begin
...
//-------------------------- PRIMER REGISTRO -----------------------------------
procedure TUnidadesMedida.Primero(Sender: TObject);
begin
...
//-------------------------- REGISTRO ANTERIOR ---------------------------------
procedure TUnidadesMedida.Anterior(Sender: TObject);
begin
...
//-------------------------- REGISTRO SIGUIENTE --------------------------------
procedure TUnidadesMedida.Siguiente(Sender: TObject);
begin
...
//-------------------------- ULTIMO REGISTRO -----------------------------------
procedure TUnidadesMedida.Ultimo(Sender: TObject);
begin
...
//----------------------------- BUSQUEDA ---------------------------------------
procedure TUnidadesMedida.Busqueda(Sender: TObject);
Var
   Aux: TBusquedaUnidadesMedida;
begin
     try
        Aux:=TBusquedaUnidadesMedida.Create(Self);
        with Aux do
        begin
             AceptarTodos:=True;
...
//------------------------ BOTON ACEPTAR ----------------------------------------
procedure TUnidadesMedida.Aceptar(Sender: TObject);
Var
   Posicion: TBookmark;
begin
...
//--------------------- BOTON CANCELAR -----------------------------------------
procedure TUnidadesMedida.Cancelar(Sender: TObject);
begin
     if QMaestro.State in dsEditModes then
     begin
...
//----------------------------- SALIR ------------------------------------------
procedure TUnidadesMedida.Salir(Sender: TObject);
begin
     Close;
end;

//-------------------------- VALIDACION DE DATOS CABECERA ----------------------
function TUnidadesMedida.ValidarCabecera():Boolean;
begin
      Result:=True;
...
function TUnidadesMedida.ValidarDetalles():Boolean;
begin
     Result:=True;
end;

//------------------- FOCO EN EL PANEL DE DATOS --------------------------------
procedure TUnidadesMedida.PanelDatosEnter(Sender: TObject);
begin
...
function TUnidadesMedida.EjecutaQuery(Empresa, Codigo: String):Boolean;
begin
     Result:=False;
     NRegistros:=0;
     PosRegistro:=0;
     try
        QMaestro.Close;
        QMaestro.SQL.Clear;
        QMaestro.SQL.Add(' SELECT * FROM UNIDADES ' +
                         ' WHERE EMPRESA_U=' + QuotedStr(Empresa) +
                         ' AND CODIGO_U=' + QuotedStr(Codigo) +
                         ' ORDER BY EMPRESA_U, CODIGO_U');
        QMaestro.Open;
        if QMaestro.IsEmpty then
           Result:=False
        else
        begin
             try    //Contamos los Registros
                ...
                begin
                     Close;
                     SQL.Clear;
                     SQL.Add(' SELECT COUNT(*) AS NREGISTROS FROM UNIDADES ' +
                             ' WHERE EMPRESA_U=' + QuotedStr(Empresa) +
                             ' AND CODIGO_U=' + QuotedStr(Codigo));
                     Open;
...
function TUnidadesMedida.EjecutaQueryBus(WhereBus: String):Boolean;
begin
     Result:=False;
     NRegistros:=0;
     PosRegistro:=0;
        ...
        begin
             try    //Contamos los Registros
                ...
                begin
                     Close;
                     SQL.Clear;
                     SQL.Add('SELECT COUNT(*) AS NREGISTROS FROM UNIDADES ' + WhereBus);
                     Open;
                     NRegistros:=FieldByName('NRegistros').AsInteger;  //Contamos los registros
                     PosRegistro:=1;
                end;
...
//-------------------- EVENTO SCROLL -------------------------------------------
procedure TUnidadesMedida.AlMoverRegistro(DataSet: TDataSet);
begin
     if SwAlta=False then  //El estado es de visualizacion
     begin
          cEmpresa.Text:=QMaestro.FieldByName('EMPRESA_U').AsString; //No estan enganchados a BD y debemos asignarlos a mano
          cCodigo.Text:=QMaestro.FieldByName('CODIGO_U').AsString;
     end;
end;

//--------------------- CAMBIO DE ESTADO DEL DATA SOURCE -----------------------
procedure TUnidadesMedida.DSMaestroStateChange(Sender: TObject);
begin
...
procedure TUnidadesMedida.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
//--------------------- CONTROL DE PULSACIONES DE TECLADO ----------------------
procedure TUnidadesMedida.FormKeyPress(Sender: TObject; var Key: Char);
begin
     if Key=#13 then
     begin
          Key:=#0;
...
//---------------------- DESPUES DE INSERTAR -----------------------------------
procedure TUnidadesMedida.QMaestroAfterInsert(DataSet: TDataSet);
begin
     QMaestro.FieldByName('EMPRESA_U').AsString:=cEmpresa.Text;
     QMaestro.FieldByName('CODIGO_U').AsString:=cCodigo.Text;
end;

//---------------------- PONE EL COLOR DE FOCO DE LOS CAMPOS CLAVES ------------
procedure TUnidadesMedida.ColorEnter(Sender: TObject);
begin
...
procedure TUnidadesMedida.ColorExit(Sender: TObject);
begin
     if Sender is TEdit then           //Si el componente es un TDBEdit
     begin
          if not SwAlta then
...
procedure TUnidadesMedida.QuitarColorAlta();
begin
...
procedure TUnidadesMedida.PonerColorAlta();
begin
...
procedure TUnidadesMedida.cEmpresaChange(Sender: TObject);
begin
     tNomEmpresa.Caption:=NomEmpresa(RellenarCeros(cEmpresa.Text,cEmpresa.MaxLength,'I'));
end;

//-------------------------- LUPA (F2) DE EMPRESAS -----------------------------
procedure TUnidadesMedida.cEmpresaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
     if Key=VK_F2 then
...
//-------------------------- LUPA DE EMPRESAS ----------------------------------
procedure TUnidadesMedida.cLupaEmpresasClick(Sender: TObject);
Var
   CodEmpresa: String;
begin
...
procedure TUnidadesMedida.FormActivate(Sender: TObject);
begin
     cCodigo.SetFocus;
end;
